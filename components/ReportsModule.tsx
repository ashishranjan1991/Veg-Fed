...
// Moved FilterSection outside to fix TS children prop errors and avoid unnecessary re-renders
...
const ReportsModule: React.FC<ReportsModuleProps> = ({ role, language }) => {
  ...
  // Quality Distribution Data (SRS Page 67)
  const qualityData = [
  ...
  // Scheme Utilization Trend (SRS Page 73)
    ...
    <div className="animate-in fade-in duration-500 pb-20">
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-slate-400 text-sm mt-1 font-medium">Bihar State Vegetable Scheme Distributed Analytics & Support (FRS-08)</p>
        </div>
      ...
      {activeTab === 'analytics' ? (
        <div className="space-y-8">
          {/* Executive Dashboard View (FRS-08) */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-slate-800 shadow-sm h-[400px]">
               <h3 className="text-sm font-black uppercase text-gray-400 mb-6 flex items-center">
                 <i className="fa-solid fa-microscope mr-2 text-emerald-500"></i> Quality Distribution (All India)
               </h3>
               <ResponsiveContainer width="100%" height="80%">
                  <PieChart>
                    <Pie
                ...
                <div className="animate-in slide-in-from-right-12 duration-500 h-full flex flex-col">
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mt-1">Data: {filters.startDate} to {filters.endDate}</p>
                    </div>
        ...
        <div className="animate-in fade-in duration-500">
           <div className="bg-white dark:bg-slate-900 rounded-[3rem] border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden p-8">
              <h3 className="text-xl font-black uppercase tracking-tight dark:text-white mb-8">Active Support Queue</h3>
                 ...
                 {tickets.map(ticket => (
                    <div key={ticket.id} className="p-6 bg-gray-50 dark:bg-slate-800/50 rounded-3xl border border-gray-100 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all hover:border-emerald-500">
                       <div className="flex items-start space-x-5">
                          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-black bg-amber-100 text-amber-600`}>
                          ...
                          <div>
                             <div className="flex items-center space-x-3">
                                ...
                                <span className="text-[9px] text-gray-400 font-bold uppercase bg-white dark:bg-slate-900 px-2 py-0.5 rounded border dark:border-slate-700">{ticket.category}</span>
                             </div>
                             <p className="text-sm text-gray-500 mt-1 font-medium">{ticket.message}</p>
                             <p className="text-[10px] text-gray-400 font-bold mt-2"><i className="fa-solid fa-clock mr-1"></i> {ticket.createdAt}</p>
                          </div>
                       </div>
                       <select value={ticket.status} className="bg-white dark:bg-slate-900 border dark:border-slate-700 rounded-xl px-4 py-2 text-[10px] font-black uppercase tracking-widest outline-none">
