...
const Home: React.FC<HomeProps> = ({ onLogin, language, setLanguage, fontSize, setFontSize, theme, setTheme }) => {
  ...
  const handleRecoveryStart = () => setLoginSubView('forgot-otp');
  const handleRecoveryVerify = () => setLoginSubView('forgot-reset');
  ...
  const newsItems = [
    ...
    { date: "08 Jan 2026", title: "Tomato Exports to Nepal Begin", desc: "Tarkaari brand produce from Tirhut Union has successfully crossed borders." },
    { date: "05 Jan 2026", title: "Quality Grading Workshop at Patna Union", desc: "Training for PVCS officials on new digital grading parameters concluded." }
  ];
  ...
  return (
    <div className={`min-h-screen bg-white dark:bg-slate-950 flex flex-col font-size-${fontSize} transition-colors duration-300`}>
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-2 md:space-x-4 shrink-0">
             ...
             <div className="flex items-center space-x-2 border-l border-gray-200 dark:border-slate-800 pl-2 md:pl-4">
               <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-lg bg-gray-100 dark:bg-slate-800 text-gray-500 hover:text-emerald-600 transition-colors">
      ...
      {/* Hero Section */}
      <section className="relative py-20 px-6 overflow-hidden min-h-[650px] flex items-center">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
          <div className="lg:col-span-7 text-white animate-in fade-in slide-in-from-left-8 duration-700">
            ...
            <h2 className="text-6xl lg:text-8xl font-black mb-8 leading-none tracking-tighter drop-shadow-2xl">{t.slogan}</h2>
          ...
          <div className="lg:col-span-5 animate-in fade-in slide-in-from-right-8 duration-700">
            <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-[3rem] p-10 shadow-2xl border-4 border-emerald-500/20 transition-colors">
              <div className="flex bg-gray-100 dark:bg-slate-800 p-1.5 rounded-2xl mb-8">
                <button onClick={() => {setTab('login'); setLoginSubView('form');}} className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all ${tab === 'login' ? 'bg-white dark:bg-slate-700 text-emerald-700 dark:text-emerald-400 shadow-sm' : 'text-gray-400'}`}>{t.signIn}</button>
              ...
              {tab === 'login' ? (
                  ...
                  {loginSubView === 'form' ? (
                    <div className="space-y-5">
                      <div className="grid grid-cols-2 gap-2 mb-4">
                        <button onClick={() => setLoginRole(UserRole.FARMER)} className={`py-3 text-[10px] font-black uppercase rounded-lg border transition-all ${loginRole === UserRole.FARMER ? 'bg-emerald-600 text-white border-emerald-600' : 'text-gray-400 border-gray-200'}`}>Farmer</button>
                        <button onClick={() => setLoginRole(UserRole.PVCS_USER)} className={`py-3 text-[10px] font-black uppercase rounded-lg border transition-all ${loginRole !== UserRole.FARMER ? 'bg-emerald-600 text-white border-emerald-600' : 'text-gray-400 border-gray-200'}`}>Official</button>
                      </div>
                      <div>
                        <label className="text-[10px] font-black text-gray-400 uppercase ml-1 tracking-widest">{t.userId}</label>
                        <input type="text" className="w-full bg-gray-50/50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl px-5 py-4 mt-1 outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white" />
                      </div>
                      <div>
                        ...
                        <input type="password" className="w-full bg-gray-50/50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl px-5 py-4 mt-1 outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white" />
                      </div>
                      <button onClick={() => onLogin(loginRole)} className="w-full bg-emerald-600 text-white py-4 rounded-xl font-black text-sm uppercase tracking-widest hover:bg-emerald-700 shadow-xl transition-all">
                        {t.loginButton}
                      </button>
                    </div>
                    ...
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300 text-center">
                       ...
                       <input type="tel" value={recoveryMobile} onChange={e => setRecoveryMobile(e.target.value)} placeholder="+91 XXXXX XXXXX" className="w-full bg-gray-50/50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl px-5 py-4 mt-1 outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white font-bold" />
                    ...
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300 text-center">
                       ...
                       <div className="flex justify-center gap-2">
                          {[1,2,3,4,5,6].map(i => <input key={i} type="text" maxLength={1} className="w-10 h-12 bg-gray-50 dark:bg-slate-800 border-2 dark:border-slate-700 rounded-lg text-center font-black text-emerald-600 focus:border-emerald-500 outline-none" />)}
                       </div>
                    ...
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300 text-center">
                       ...
                       <div className="space-y-4">
                          <input type="password" placeholder="New Password" className="w-full bg-gray-50/50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl px-5 py-4 mt-1 outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white" />
                          <input type="password" placeholder="Confirm Password" className="w-full bg-gray-50/50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl px-5 py-4 mt-1 outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white" />
                       </div>
                ...
                <div className="space-y-6 animate-in fade-in duration-300">
                  <div>
                    ...
                    <input type="text" value={dbtId} onChange={e => setDbtId(e.target.value)} placeholder="e.g. 231XXXXXXXX" className="w-full bg-gray-50/50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl px-5 py-4 mt-1 outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white" />
                  </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
           ...
           <div className="space-y-8">
              ...
              <p className="text-gray-600 dark:text-slate-400 text-lg leading-relaxed font-medium">
                {t.visionStatement.substring(0, 300)}...
              </p>
      ...
      <section className="py-24 px-6 bg-gray-50 dark:bg-slate-950">
        <div className="max-w-7xl mx-auto">
           ...
           <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {unions.map((union, i) => (
                <div key={i} className="group p-8 rounded-[2rem] border-2 border-transparent bg-white dark:bg-slate-900 hover:border-emerald-500 transition-all text-center shadow-sm hover:shadow-xl">
      ...
      {/* Footer */}
      <footer className="bg-slate-950 text-slate-500 pt-24 pb-12 px-6 border-t-[12px] border-emerald-600">
        ...
        <div className="max-w-7xl mx-auto border-t border-slate-900 mt-20 pt-12 text-center">
           <p className="text-[10px] uppercase font-black tracking-widest text-slate-700">© 2025 VEGFED BIHAR | TARKAARI.IN</p>
        </div>
      </footer>
      <style>{`@keyframes marquee { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } } .animate-marquee { display: inline-block; animation: marquee 30s linear infinite; } @keyframes bounce-slow { 0%, 100% { transform: translateY(-5%); } 50% { transform: translateY(0); } } .animate-bounce-slow { animation: bounce-slow 4s ease-in-out infinite; }`}</style>
    </div>
  );
};
